...
**Headline:** QAT+VQ gives the **highest accuracy of all variants** while being
the **smallest** — 5.0× smaller than baseline and 21% smaller than PTQ, with
accuracy *above* both baseline and PTQ. The vector-quantized codebook acts as a
mild regulariser, which is why accuracy nudges slightly above the fp32 baseline.
...
|---|---|---|
| QAT+VQ accuracy | 81.17% (worst) | 91.28% (best) |
| vs PTQ | −5.4% & not smaller | **+0.11% & 21% smaller** |
| Compression | 2.09× | 4.99× |
| Verdict | hybrid failed | **hybrid wins on both axes** |
...
- SST-2 is an easy binary task; DistilBERT is near-saturated, so absolute gaps
  ...
  (hybrid ≥ PTQ > QAT) is stable and is the claim.
...
- `roc_all.png`, `pr_all.png` — ROC and PR curves, all 4 models
- `cm_Baseline.png`, `cm_PTQ.png`, `cm_QATINT8.png`, `cm_QATVQ.png` — confusion matrices
